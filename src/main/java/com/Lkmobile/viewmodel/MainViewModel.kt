...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun checkGameStatus() {
        ...
        // Auto-request permission if missing and game is installed
        if (!GameDetector.hasUsageStatsPermission(context) && GameDetector.isGameInstalled(context)) {
            // We can't easily request from here without UI, but we can set an error or flag
            // For now, let's just use the fallback which is less reliable but won't crash
        }
